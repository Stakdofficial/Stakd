...
class FakeHook:
    def __init__(self, defend_pending=None, has_defend=True):
        self.calls = []
...
def keeper_with(hook, min_burn_eth=0.00001):
    k = Keeper.__new__(Keeper)  # skip __init__: no RPC, no Lighter
    k.chain = FakeChain(hook)
    k.cfg = SimpleNamespace(min_burn_eth=min_burn_eth)
    k._no_defend_bucket = False
    return k
...
def test_old_hook_without_defend_mode_is_asked_once():
    ...
    assert k._no_defend_bucket is True
    assert k.chain.sent == []
